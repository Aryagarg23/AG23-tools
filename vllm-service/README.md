# WSL 2 vLLM Background Server (`vllm-service/`)
...
## Notes
*   Runs on the prebuilt FlashAttention backend, so WSL does not need a CUDA toolkit. The installer removes FlashInfer for this reason.
*   fp8 KV cache is off. It needs an SM89+ GPU (RTX 40-series or newer); older cards fall back to bf16, which uses more memory. See `RCA.md`.
